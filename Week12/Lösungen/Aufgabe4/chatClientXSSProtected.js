...
const getMessagesHtmlString = function() {
    return messages.map((message) => `<li>${DOMPurify.sanitize(message)}</li>`).join('');
};
...
const updateView = function() {
	...
	// safe when used with Handlebars or DOMPurify
	messagesListElement.innerHTML = getMessagesHtmlString();
	// safer: assign textContent (auto-escaped)
	// messagesListElement.textContent = getMessagesHtmlString();
...
const updateChatFromServer = async function (catId) {
...
const joinChat = async function(event) {
	...
	pushService.subscribeToChatUpdates(chatId, handle, updateChatFromServer);
    } catch {
	joinError = true;
    }
    updateView();
...
const subscribeToChatUpdates = function (chatId, handle, chatUpdatesListenerFn) {
    socket.emit('subscribe', {chatId, handle});
    socket.on('chat updated', function(data){
        console.log("socket received chat updated "+ data);
        chatUpdatesListenerFn(chatId, data)
    });
}
window.pushService = {subscribeToChatUpdates};
